...
function(link_rnnoise target_name)
    if (TG_OWT_PACKAGED_BUILD)
        find_package(PkgConfig)
        if (PkgConfig_FOUND)
            pkg_check_modules(RNNOISE rnnoise)
            if (RNNOISE_FOUND)
                target_include_directories(${target_name} SYSTEM PRIVATE ${RNNOISE_INCLUDE_DIRS})
                target_link_libraries(${target_name} PRIVATE ${RNNOISE_LINK_LIBRARIES})
            endif()
        endif()
    endif()

    if (NOT RNNOISE_FOUND)
        target_link_libraries(${target_name} PRIVATE tg_owt::rnnoise)
    endif()
endfunction()
    ...
    if (TG_OWT_PACKAGED_BUILD)
        ...
        if (PkgConfig_FOUND)
            ...
            if (LIBYUV_FOUND)
                ...
                target_link_libraries(${target_name} PRIVATE ${LIBYUV_LINK_LIBRARIES})
            endif()
        endif()
    endif()
